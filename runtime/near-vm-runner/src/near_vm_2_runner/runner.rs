...
use near_vm_2_engine::universal::{
    MemoryPool, Universal, UniversalArtifact, UniversalEngine, UniversalExecutable,
    UniversalExecutableRef,
};
use near_vm_2_types::{FunctionIndex, InstanceConfig, MemoryType, Pages, WASM_PAGE_SIZE};
use near_vm_2_vm::{
...
fn get_entrypoint_index(
    ...
    if let Some(near_vm_2_types::ExportIndex::Function(index)) = artifact.export_field(method_name)
    {
...
fn translate_runtime_error(
    error: near_vm_2_engine::RuntimeError,
    logic: &mut VMLogic,
...
impl NearVM {
    pub(crate) fn new_for_target(config: Arc<Config>, target: near_vm_2_compiler::Target) -> Self {
    ...
    pub(crate) fn new(config: Arc<Config>) -> Self {
        use near_vm_2_compiler::{CpuFeature, Target, Triple};
    ...
    fn run_method(
        ...
        assert_eq!(
            ...
            size_of::<near_vm_2_types::FastGasCounter>() + size_of::<u64>()
        );
        assert_eq!(
            offset_of!(FastGasCounter, burnt_gas),
            offset_of!(near_vm_2_types::FastGasCounter, burnt_gas)
        );
        assert_eq!(
            offset_of!(FastGasCounter, gas_limit),
            offset_of!(near_vm_2_types::FastGasCounter, gas_limit)
        );
        ...
        unsafe {
            let instance = {
                ...
                let handle = match maybe_handle {
                    Ok(handle) => handle,
                    Err(err) => {
                        use near_vm_2_engine::InstantiationError::*;
                        let abort = match err {
                ...
                match handle.finish_instantiation() {
                    ...
                    Err(trap) => {
                        let abort = translate_runtime_error(
                            near_vm_2_engine::RuntimeError::from_trap(trap),
                            import.vmlogic,
                        )?;
                        return Ok(Err(abort));
                    }
...
impl near_vm_2_vm::Tunables for &NearVM {
    ...
    fn table_style(&self, _table: &near_vm_2_types::TableType) -> near_vm_2_vm::TableStyle {
        near_vm_2_vm::TableStyle::CallerChecksSignature
    }
    ...
    unsafe fn create_vm_memory(
        ...
        ty: &MemoryType,
        _style: &MemoryStyle,
        _vm_definition_location: std::ptr::NonNull<near_vm_2_vm::VMMemoryDefinition>,
    ) -> Result<std::sync::Arc<LinearMemory>, near_vm_2_vm::MemoryError> {
    ...
    fn create_host_table(
        &self,
        _ty: &near_vm_2_types::TableType,
        _style: &near_vm_2_vm::TableStyle,
    ) -> Result<std::sync::Arc<dyn near_vm_2_vm::Table>, String> {
        panic!("should never be called")
    }

    unsafe fn create_vm_table(
        &self,
        ty: &near_vm_2_types::TableType,
        style: &near_vm_2_vm::TableStyle,
        vm_definition_location: std::ptr::NonNull<near_vm_2_vm::VMTableDefinition>,
    ) -> Result<std::sync::Arc<dyn near_vm_2_vm::Table>, String> {
    ...
    fn gas_cfg(&self) -> Box<dyn wp::VisitSimdOperator<Output = Fee>> {
...
impl<'a> wp::VisitSimdOperator<'a> for GasCostCfg {
...
near_vm_types! {
    u32 as i32 => near_vm_2_types::Type::I32;
    u64 as i64 => near_vm_2_types::Type::I64;
}
...
impl<'e, 'l, 'lr> Resolver for NearVmImports<'e, 'l, 'lr> {
    fn resolve(&self, _index: u32, module: &str, field: &str) -> Option<near_vm_2_vm::Export> {
        if module == "env" && field == "memory" {
            return Some(near_vm_2_vm::Export::Memory(self.memory.clone()));
        }
                ...
                (
                  ...
                  >
                    ...
                    -> Ret {
                        ...
                        match result {
                            ...
                            Ok(Err(trap)) => unsafe {
                                ...
                                near_vm_2_vm::raise_user_trap(Box::new(trap))
                            },
                            Err(e) => unsafe {
                                // SAFETY: this can only be called by a WASM contract, so all the
                                // necessary hooks are known to be in place.
                                near_vm_2_vm::resume_panic(e)
                            },
                        }
                    }
                    ...
                    if module == stringify!($mod) && field == stringify!($name) {
                        ...
                        let signature = near_vm_2_types::FunctionType::new(&args[..], &rets[..]);
                        let signature = self.engine.register_signature(signature);
                        return Some(near_vm_2_vm::Export::Function(ExportFunction {
                            vm_function: VMFunction {
                                ...
                                vmctx: near_vm_2_vm::VMFunctionEnvironment {
