...
pub fn finite_wasm_stack(
    ...
    let Ctx { result_state, remaining_stack, config, .. } = caller.data_mut();
    *remaining_stack = match remaining_stack.checked_sub(operand_size.saturating_add(frame_size)) {
    ...
    let gas = ((frame_size + 7) / 8) * u64::from(config.regular_op_cost);
...
pub fn write_register(
    ...
    let (memory, Ctx { config, registers, result_state, .. }) = memory.data_and_store_mut(caller);
    result_state.gas_counter.pay_base(base)?;
    let memory = read_memory(&mut result_state.gas_counter, memory, data_ptr, data_len)?;
    registers.set(&mut result_state.gas_counter, &config.limit_config, register_id, memory)
}
...
pub fn alt_bn128_g1_multiexp(
    ...
    let (memory, Ctx { registers, result_state, config, .. }) = memory.data_and_store_mut(caller);
    ...
    registers.set(&mut result_state.gas_counter, &config.limit_config, register_id, res)
}
